...
drop policy if exists "quote_presets_select_own" on public.quote_presets;
create policy "quote_presets_select_own"
on public.quote_presets for select
using (editor_id = auth.uid());

drop policy if exists "quote_presets_insert_own" on public.quote_presets;
create policy "quote_presets_insert_own"
on public.quote_presets for insert
with check (editor_id = auth.uid());

drop policy if exists "quote_presets_update_own" on public.quote_presets;
create policy "quote_presets_update_own"
on public.quote_presets for update
using (editor_id = auth.uid())
with check (editor_id = auth.uid());

drop policy if exists "quote_presets_delete_own" on public.quote_presets;
create policy "quote_presets_delete_own"
